      ...
      <div className="container mx-auto px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <div className="space-y-6">
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-yellow-400">
            Tentang Kaizen Network
          </h2>

          <p className="text-gray-300 text-lg leading-relaxed max-w-lg transition-all duration-300">
            {displayedText}
          </p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-2 gap-4">
            {features.map((feature, i) => (
                <div
                    key={i}
                    className="flex flex-col items-center justify-center p-6 rounded-2xl border border-gray-800
                    ...
                    style={{
                        transitionTimingFunction: 'cubic-bezier(0.34, 1.56, 0.64, 1)', // bounce lembut
                    }}
                >
                    <div className="p-3 bg-[#f8cb08]/10 rounded-xl text-[#f8cb08] mb-4">
                        {feature.icon}
                    </div>
                    <p className="font-medium text-sm text-gray-300">{feature.title}</p>
                </div>
            ))}
        </div>
      </div>
